...
if (isset($_GET['id'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $conn->prepare("UPDATE users SET name = ?, email = ?, user_type = ?, phone = ?, address = ?, image = ? WHERE id = ?");
        $stmt->execute([$name, $email, $user_type, $phone, $address, $new_image_name, $id]);

        echo '<div style="background-color: #d4edda; color: #155724; padding: 10px 15px; font-size: 18px; border-radius: 5px; margin-top: 10px;">
        <center> User updated successfully.</center>
        </div>';
...
<section class="product-form">
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <input type="text" name="phone" class="box" value="<?= htmlspecialchars($user['phone']) ?>">

        <p>Address<span>*</span></p>
        <input type="text" name="address" class="box" value="<?= htmlspecialchars($user['address']) ?>">
